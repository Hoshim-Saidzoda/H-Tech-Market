...
const CategoryList: React.FC<Props> = ({ onSelect }) => {
  ...
  const handleCategoryInteraction = (catId: number) => {
    setActiveId(catId);
    if (isMobile) {
  ...
  return (
    <div className={`${isMobile ? 'flex flex-col' : 'grid grid-cols-12 gap-8'}`}>
       <div className={`${isMobile ? 'w-full' : 'col-span-3 border-r pr-4'}`}>
        ...
        <ul className="space-y-2">
          {categories?.map((cat) => {
            ...
            return (
              <li
                key={cat.id}
                onClick={() => handleCategoryInteraction(cat.id)}
                onMouseEnter={!isMobile ? () => setActiveId(cat.id) : undefined}
      ...
      <div className={`${isMobile ? 'w-full' : 'col-span-9'}`} ref={subcategoryRef}>
        {activeCategory ? (
          <div className={`${isMobile ? 'mt-8' : 'grid sticky top-14 grid-cols-12 gap-8'}`}>
            <div className={`${isMobile ? '' : 'col-span-7'}`}>
              <div className={`${isMobile ? '' : 'sticky top-6'}`}>
                <h2 className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold mb-6 text-gray-800`}>
                    ...
                    {activeCategory.subCategories.map((sub) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          {isMobile && (
                            <ChevronRight sx={{ fontSize: 18, color: 'grey.400' }} />
                          )}
                        </div>
                        {sub.description && (
            ...
            {activeCategory.categoryImage && (
              <div className={`${isMobile ? 'mt-8' : 'col-span-5'}`}>
                <img
                  src={`https://store-api.softclub.tj/images/${activeCategory.categoryImage}`}
                  alt={activeCategory.categoryName}
                  className={`w-full ${isMobile ? 'h-48' : 'h-72'} object-cover rounded-2xl shadow-lg transition-transform duration-500 hover:scale-105`}
                />
              </div>
            )}
          </div>
          ...
          <p className="text-gray-400">
            {isMobile ? "Нажмите на категорию выше" : "Наведите курсор на категорию"}
          </p>
        )}
      </div>
    </div>
